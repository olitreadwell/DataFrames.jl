...
@testset "scalar on assignment side" begin
    ...
    if VERSION >= v"1.14.0-DEV"
        @test_throws ArgumentError df[1, 1] .= df[1, 1] .- df[1, 1]
    else
    ...
    @test df[1, 1] == 0
    if VERSION >= v"1.14.0-DEV"
        @test_throws ArgumentError df[1, 2] .-= df[1, 2]
    else
...
@testset "additional checks of post-! broadcasting rules" begin
    if VERSION >= v"1.14.0-DEV"
        df = copy(refdf)
        v1 = df[!, 1]
        @test_throws ArgumentError df[CartesianIndex(1, 1)] .= 1
        @test_throws ArgumentError df[CartesianIndex(1, 1)] .= "d"
        @test_throws ArgumentError df[CartesianIndex(1, 1)] .= [1, 2]

        df = copy(refdf)
        v1 = df[!, 1]
        @test_throws ArgumentError df[1, 1] .= 1
        @test_throws ArgumentError df[1, 1] .= "d"
        @test_throws ArgumentError df[1, 1] .= [1, 2]

        df = copy(refdf)
        v1 = df[!, 1]
        @test_throws ArgumentError df[1, :x1] .= 1
        @test_throws ArgumentError df[1, :x1] .= "d"
        @test_throws ArgumentError df[1, :x1] .= [1, 2]
...
@testset "add new correct rules for df[row, col] .= v broadcasting" begin
    for v in [:a, "a"]
        df = DataFrame(a=1)
        if VERSION >= v"1.14.0-DEV"
            @test_throws ArgumentError df[1, 1] .= 10
            @test_throws ArgumentError df[1, v] .= 10
            @test_throws ArgumentError df[CartesianIndex(1, 1)] .= 10
        else
